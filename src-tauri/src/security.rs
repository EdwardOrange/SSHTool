...
pub fn redact(input: &str) -> String {
    ...
    for marker in ["password=", "passwd=", "token=", "secret=", "Authorization:"] {
        let marker_lower = marker.to_lowercase(); let mut offset = 0;
        while offset < result.len() {
            let Some(relative) = result[offset..].to_lowercase().find(&marker_lower) else { break; };
            let marker_start = offset + relative; let value_start = marker_start + marker.len();
            let end = result[value_start..].find(|c: char| c.is_whitespace() || c == '&' || c == ';').map(|i| value_start + i).unwrap_or(result.len());
            result.replace_range(value_start..end, "[REDACTED]"); offset = value_start + "[REDACTED]".len();
        }
    }
    result
}
...
#[cfg(test)]
mod tests { use super::*; #[test] fn quotes_shell_values() { assert_eq!(shell_quote("a'b").unwrap(), "'a'\\''b'"); } #[test] fn rejects_newlines() { assert!(shell_quote("a\nb").is_err()); } #[test] fn redacts_secrets() { assert!(!redact("password=hunter2 token=abc").contains("hunter2")); } }
