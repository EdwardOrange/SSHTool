...
export default function ForwardingView({ host }: { host: HostProfile }) {
  ...
  React.useEffect(() => {
    ...
    setProfiles([]); setError(""); setPending([]); setSaving(false); setLoading(true); setDeleteTarget(undefined);
    api.forwardingList(host.id).then((items) => { if (current === generation.current) setProfiles(items); })
  ...
  const save = async () => {
    ...
    try {
      const saved = await api.forwardingUpsert({ ...draft, id: draft.id || crypto.randomUUID(), hostId: host.id });
      if (current !== generation.current) return;
      setProfiles((items) => [...items.filter((item) => item.id !== saved.id), saved]); setAdding(false);
    } catch (e) { if (current === generation.current) setError(formatError(e)); }
    finally { if (current === generation.current) setSaving(false); }
  };
  const toggle = async (profile: ForwardingProfile) => {
    if (pending.includes(profile.id)) return;
    const current = generation.current;
    setPending((items) => [...items, profile.id]);
    try {
      ...
      if (current === generation.current) { setError(formatError(e)); setProfiles((items) => items.map((item) => item.id === profile.id ? { ...profile, lastError: formatError(e) } : item)); }
    } finally { if (current === generation.current) setPending((items) => items.filter((id) => id !== profile.id)); }
  };
  const remove = async (profile: ForwardingProfile) => { setDeleteTarget(profile); };
  const performRemove = async () => { const profile = deleteTarget; const current = generation.current; setDeleteTarget(undefined); if (!profile) return; setPending((items) => [...items, profile.id]); try { await api.forwardingDelete(profile.id); if (current === generation.current) setProfiles((items) => items.filter((item) => item.id !== profile.id)); } catch (e) { if (current === generation.current) setError(formatError(e)); } finally { if (current === generation.current) setPending((items) => items.filter((id) => id !== profile.id)); } };
  return <Stack sx={{ height: "100%", overflow: "auto" }} spacing={1.5}><Stack direction="row" alignItems="center"><AltRouteRounded color="primary" sx={{ mr: 1 }}/><Typography variant="h6">端口转发</Typography><Box sx={{ flex: 1 }}/><Button startIcon={<AddRounded/>} variant="contained" disabled={loading || saving} onClick={() => setAdding(!adding)}>新建转发</Button></Stack>{error && <Alert severity="error" onClose={() => setError("")}>{error}</Alert>}
    {adding && <Paper variant="outlined" sx={{ p: 2 }}><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2, minmax(0, 1fr))", xl: "repeat(3, minmax(0, 1fr))" }, gap: 2 }}><TextField size="small" label="名称" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })}/><TextField size="small" select label="类型" value={draft.kind} onChange={(e) => setDraft({ ...draft, kind: e.target.value as ForwardingProfile["kind"] })}><MenuItem value="local">本地 -L</MenuItem><MenuItem value="remote">远程 -R</MenuItem><MenuItem value="dynamic">SOCKS -D</MenuItem></TextField><TextField size="small" label="监听地址" value={draft.bindAddress} onChange={(e) => setDraft({ ...draft, bindAddress: e.target.value })}/><TextField size="small" type="number" label="监听端口" value={draft.bindPort} onChange={(e) => setDraft({ ...draft, bindPort: Number(e.target.value) })}/>{draft.kind !== "dynamic" && <><TextField size="small" label="目标主机" value={draft.targetHost} onChange={(e) => setDraft({ ...draft, targetHost: e.target.value })}/><TextField size="small" type="number" label="目标端口" value={draft.targetPort} onChange={(e) => setDraft({ ...draft, targetPort: Number(e.target.value) })}/></>}<Button disabled={saving || loading} variant="contained" onClick={save}>{saving ? "正在保存…" : "保存"}</Button></Box></Paper>}
