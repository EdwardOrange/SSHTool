...
export default function MonitorView({ host }: { host: HostProfile }) {
  ...
  const stale = !current || Date.now() - new Date(current.timestamp).getTime() > interval * 3000;
  return <Box sx={{ overflowY: "auto", height: "100%", pr: .5 }}>
    ...
    <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 190px), 1fr))", gap: 1.5 }}>{[
      ...
      <Card variant="outlined" sx={{ flex: 1, minWidth: 220 }}><CardContent><Stack direction="row" alignItems="center" spacing={1}><MemoryRounded color="primary" /><Typography variant="subtitle2">网络与连接</Typography></Stack><Stack direction="row" spacing={2} sx={{ mt: 1.4 }}><Stack><Stack direction="row" spacing={.5}><CloudDownloadRounded fontSize="small" color="success"/><Typography variant="caption">接收</Typography></Stack><Typography variant="h6">{current ? formatBytes(current.rxBytesPerSec, true) : "—"}</Typography></Stack><Stack><Stack direction="row" spacing={.5}><CloudUploadRounded fontSize="small" color="primary"/><Typography variant="caption">发送</Typography></Stack><Typography variant="h6">{current ? formatBytes(current.txBytesPerSec, true) : "—"}</Typography></Stack></Stack><Typography variant="caption" color="text.secondary">{current ? `${current.connectionCount} 个活动连接` : "等待数据"}</Typography></CardContent></Card>
    </Box>
...
function MetricCard({ label, value, detail, color }: { label: string; value?: number; detail: string; color: string }) { return <Card variant="outlined" sx={{ flex: 1, minWidth: 190 }}><CardContent><Stack direction="row" justifyContent="space-between"><Typography variant="subtitle2">{label}</Typography>{value === undefined ? <Skeleton width={58} height={35} /> : <Typography variant="h5" sx={{ color }}>{value.toFixed(1)}%</Typography>}</Stack>{value === undefined ? <Skeleton sx={{ my: 1.3 }} /> : <LinearProgress variant="determinate" value={value} sx={{ my: 1.3, height: 7, borderRadius: 8, "& .MuiLinearProgress-bar": { bgcolor: color } }} />}<Typography variant="caption" color="text.secondary">{detail}</Typography></CardContent></Card>; }
