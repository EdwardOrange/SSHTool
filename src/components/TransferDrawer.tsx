...
export default function TransferDrawer() {
  ...
  return <Paper square variant="outlined" sx={{ borderLeft: 0, borderRight: 0, borderRadius: 0, zIndex: 2 }}>
    ...
    <Collapse in={expanded}><Divider/><Stack spacing={1} sx={{ px: 1.5, py: 1, maxHeight: "min(230px, 20vh)", overflow: "auto" }}>{items.map((task) => <TransferRow key={task.progress.transferId} task={task}/>)}</Stack></Collapse>
  </Paper>;
}
